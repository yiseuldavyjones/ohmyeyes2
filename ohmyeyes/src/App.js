    ...
    const checkUV = async () => {
        ...
        try {
            ...
            if (geoData && geoData.length > 0) {
                const locationName = geoData[0].local_names?.ko || geoData[0].name;
                setLocation(locationName);
            }

            // API 호출 
            const res = await fetch(`https://api.openweathermap.org/data/2.5/uvi?lat=${lat}&lon=${lon}&appid=${API_KEY}`);
    ...
    return (
        ...
        >
            ...
            >
                ...
                <div style={{ fontSize: "20px", margin: "20px 0", fontWeight: "bold" }}>{uv === null ? "자외선을 확인" : Number(uv) < 3 ? "선글라스 ㄴㄴ" : "🕶️ 선글라스 ㄱㄱ"}</div>
                ...
                >
                    <strong>기준:</strong> 0-2 낮음 | 3-5 보통 | 6+ 높음
                </div>
            </div>
        </div>
    );
